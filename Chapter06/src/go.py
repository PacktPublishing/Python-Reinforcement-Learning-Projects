...
(0, 0) is considered to be the upper left corner of the board, and (N, 0) is the lower left.
'''
import copy
import itertools
...
import utils
from config import GOPARAMETERS, NEIGHBORS, DIAGONALS

def initialize_board():
    return np.zeros([GOPARAMETERS.N, GOPARAMETERS.N], dtype=np.int8)
...
class PositionWithContext(namedtuple('SgfPosition', ['board_state', 'next_move', 'result'])):
    pass
...
def replay_board_state(board_state, result):
    '''
    Wrapper for a go.Position which replays its history.
    Assumes an GOPARAMETERS.EMPTY start board_state! (i.e. no handicap, and history must be exhaustive.)

    Result must be passed in, since a resign cannot be inferred from board_state
    history alone.

    for board_state_w_context in replay_board_state(board_state):
        print(board_state_w_context.board_state)
    '''
...
def is_koish(board, c):
    ...
    if board[c] != GOPARAMETERS.EMPTY:
        return None
    neighbors = {board[n] for n in NEIGHBORS[c]}
    if len(neighbors) == 1 and not GOPARAMETERS.EMPTY in neighbors:
...
def is_eyeish(board, c):
    ...
    if len(diagonals) < 4:
        diagonal_faults += 1
    for d in diagonals:
        if not board[d] in (color, GOPARAMETERS.EMPTY):
            diagonal_faults += 1
...
class Group(namedtuple('Group', ['id', 'stones', 'liberties', 'color'])):
    ...
    liberties: a frozenset of Coordinates that are GOPARAMETERS.EMPTY and adjacent to this group.
...
class LibertyTracker():
    ...
    def from_board(board):
        ...
        for color in (GOPARAMETERS.WHITE, GOPARAMETERS.BLACK):
            while color in board:
                curr_group_id += 1
                found_color = np.where(board == color)
                coord = found_color[0][0], found_color[1][0]
                chain, reached = find_reached(board, coord)
                liberties = frozenset(r for r in reached if board[r] == GOPARAMETERS.EMPTY)
                ...
                place_stones(board, GOPARAMETERS.FILL, chain)

        lib_tracker.max_group_id = curr_group_id

        liberty_counts = np.zeros([GOPARAMETERS.N, GOPARAMETERS.N], dtype=np.uint8)
    ...
    def __init__(self, group_index=None, groups=None, liberty_cache=None, max_group_id=1):
        ...
        self.group_index = group_index if group_index is not None else - \
            np.ones([GOPARAMETERS.N, GOPARAMETERS.N], dtype=np.int32)
        self.groups = groups or {}
        self.liberty_cache = liberty_cache if liberty_cache is not None else np.zeros([GOPARAMETERS.N,
                                                                                       GOPARAMETERS.N], dtype=np.uint8)
    ...
    def add_stone(self, color, c):
        assert self.group_index[c] == GOPARAMETERS.MISSING_GROUP_ID
        captured_stones = set()
        opponent_neighboring_group_ids = set()
        friendly_neighboring_group_ids = set()
        GOPARAMETERS.EMPTY_neighbors = set()

        for n in NEIGHBORS[c]:
            neighbor_group_id = self.group_index[n]
            if neighbor_group_id != GOPARAMETERS.MISSING_GROUP_ID:
                ...
                GOPARAMETERS.EMPTY_neighbors.add(n)

        new_group = self._create_group(color, c, GOPARAMETERS.EMPTY_neighbors)
    ...
    def _capture_group(self, group_id):
        ...
        for s in dead_group.stones:
            self.group_index[s] = GOPARAMETERS.MISSING_GROUP_ID
    ...
    def _handle_captures(self, captured_stones):
        for s in captured_stones:
            for n in NEIGHBORS[s]:
                ...
                if group_id != GOPARAMETERS.MISSING_GROUP_ID:
...
class BoardState():
    ...
    def __init__(self, board=None, n=0, komi=7.5, caps=(0, 0),
                 ...
                 board_deltas=None, to_play=GOPARAMETERS.BLACK):
        '''
        board: a numpy array
        ...
        to_play: GOPARAMETERS.BLACK or GOPARAMETERS.WHITE
        '''
        ...
        self.ko = ko
        self.recent = recent
        self.board_deltas = board_deltas if board_deltas is not None else np.zeros([0, GOPARAMETERS.N,
                                                                                    GOPARAMETERS.N], dtype=np.int8)
    ...
    def __deepcopy__(self, memodict={}):
        ...
        return BoardState(new_board, self.n, self.komi, self.caps, new_lib_tracker, self.ko, self.recent,
                          self.board_deltas, self.to_play)

    def __str__(self):
        pretty_print_map = {
            GOPARAMETERS.WHITE: '\x1b[0;31;47mO',
            GOPARAMETERS.EMPTY: '\x1b[0;31;43m.',
            GOPARAMETERS.BLACK: '\x1b[0;31;40mX',
            GOPARAMETERS.FILL: '#',
            GOPARAMETERS.KO: '*',
        }
        ...
        if self.ko is not None:
            place_stones(board, GOPARAMETERS.KO, [self.ko])
        raw_board_contents = []
        for i in range(GOPARAMETERS.N):
            row = []
            for j in range(GOPARAMETERS.N):
        ...
        row_labels = ['%2d ' % i for i in range(GOPARAMETERS.N, 0, -1)]
        annotated_board_contents = [''.join(r) for r in zip(
            row_labels, raw_board_contents, row_labels)]
        header_footer_rows = [
            '   ' + ' '.join(('%s' % GOPARAMETERS.COL_NAMES)[:GOPARAMETERS.N]) + '   ']
    ...
    def is_move_legal(self, move):
        'Checks that a move is on an GOPARAMETERS.EMPTY space, not on ko, and not suicide'
        if move is None:
            return True
        if self.board[move] != GOPARAMETERS.EMPTY:
            return False
        if move == self.ko:
            return False
    ...
    def pass_move(self, mutate=False):
        ...
        pos.board_deltas = np.concatenate((
            np.zeros([1, GOPARAMETERS.N, GOPARAMETERS.N], dtype=np.int8),
            pos.board_deltas[:6]))
        pos.to_play *= -1
        pos.ko = None
        return pos
    ...
    def play_move(self, c, color=None, mutate=False):
        ...
        if not self.is_move_legal(c):
            raise IllegalMove("{} move at {} is illegal: \n{}".format(
                "GOPARAMETERS.BLACK" if self.to_play == GOPARAMETERS.BLACK else "GOPARAMETERS.WHITE",
                utils.to_kgs(c), self))
        ...
        place_stones(pos.board, GOPARAMETERS.EMPTY, captured_stones)

        opp_color = color * -1

        new_board_delta = np.zeros([GOPARAMETERS.N, GOPARAMETERS.N], dtype=np.int8)
        ...
        if pos.to_play == GOPARAMETERS.BLACK:
        ...
        pos.board_deltas = np.concatenate((
            new_board_delta.reshape(1, GOPARAMETERS.N, GOPARAMETERS.N),
            pos.board_deltas[:6]))
        pos.to_play *= -1
        return pos
    ...
    def score(self):
        ...
        while GOPARAMETERS.EMPTY in working_board:
            unassigned_spaces = np.where(working_board == GOPARAMETERS.EMPTY)
            c = unassigned_spaces[0][0], unassigned_spaces[1][0]
            territory, borders = find_reached(working_board, c)
            border_colors = set(working_board[b] for b in borders)
            X_border = GOPARAMETERS.BLACK in border_colors
            O_border = GOPARAMETERS.WHITE in border_colors
            if X_border and not O_border:
                territory_color = GOPARAMETERS.BLACK
            elif O_border and not X_border:
                territory_color = GOPARAMETERS.WHITE
            else:
                territory_color = GOPARAMETERS.UNKNOWN  # dame, or seki
            place_stones(working_board, territory_color, territory)

        return np.count_nonzero(working_board == GOPARAMETERS.BLACK) - np.count_nonzero(
            working_board == GOPARAMETERS.WHITE) - self.komi
